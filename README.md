<div class="body">
	<img class="title-image" src="https://i.imgur.com/EHgb3Eb.png"/>
	<div>
	<p>
		Training Guide to Pocket Monsters (or TGPM for short) is a Pokémon TTRPG system that focuses on general Tabletop RPG gameplay and deep character building and customization options.
	</p>
	<p class="title">
		Core Aspects
	</p>
	<p>
		&#9679; Trainer class system that makes everyone unique, with its own specialities; <br>
		&#9679; Pokémon building focused on balancing and making every pokémon have their own unique effects exclusive to their species; <br>
		&#9679; Team focused gameplay, so each player can have their own roles both in exploration and combat; <br>
	</p>
	<p>
		<b>Disclaimer:</b> The system is still in progress, as such, some aspects of all the books are still incomplete, like some Class features still not created, a lack of any tier V move or just about 80% of the pokémon are not made yet.
	</p>
	<p class="title">
		Important Links
	</p>
	<p>
		&#9679; <a href="https://github.com/Temmye/Training-Guide-to-Pocket-Monsters/blob/main/Main%20Book/Training%20Guide%20to%20Pocket%20Monsters.pdf" target="_blank"> Core Rulebook </a> <br>
		&#9679; <a href="https://github.com/Temmye/Training-Guide-to-Pocket-Monsters/blob/main/Main%20Book/TGPM%20Moves.pdf" target="_blank"> Pokémon Moves List </a> <br>
		&#9679; <a href="https://github.com/Temmye/Training-Guide-to-Pocket-Monsters/blob/main/Main%20Book/TGPM%20Pok%C3%A9dex.pdf" target="_blank"> Pokédex </a> <br>
		&#9679; <a href="https://discord.gg/wM35yEQnAw" target="_blank"> Discord Server </a> <br>
	</p>
	</div>
</div>
